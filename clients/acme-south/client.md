...
## Identity
Brand name: Acme South, and nothing else. There is no connection to any other client, past or
present. The operator-owned brand description is `clients/acme-south/description.md`. Read it as
...
Name things only as: Acme South. Never use a generic stand-in such as "the company", "the brand",
or "the product". Add every further permitted entity name to `entity_names` in
`clients/acme-south/gates.json`, which is what the gates read.
...
`clients/acme-south/never-claim.md`, one per line, and they seed that file at review time.

## Resources
`clients/acme-south/Resources/` is this client's knowledge base. Read it before any external
