...
## Identity
Brand name: Bout India, and nothing else. There is no connection to any other client, past or
present. The operator-owned brand description is `clients/bout-india/description.md`. Read it as
context, never as a citable source.

## Domain
Primary domain: https://www.boutindia.com/ . The live site wins over internal
...
## Industry reference
Industry: media-publishing. Agent W MUST read `.claude/skills/geo-content-writer/references/industries/media-publishing.md` on every run.
...
Name things only as: Bout India. Never use a generic stand-in such as "the company", "the brand",
or "the product". Add every further permitted entity name to `entity_names` in
`clients/bout-india/gates.json`, which is what the gates read.
...
## Resources
`clients/bout-india/Resources/` is this client's knowledge base. Read it before any external
