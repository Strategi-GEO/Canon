...
def dispatch_question_to_client(client_slug, topic_slug, email):
    """Case C 'Get it answered': send a PASSED blog's evaluator question to the client to answer.

    A 95+ draft ships to internal admin review carrying any question the evaluator raised (see
    runner._resolve_needs_review). The admin can weigh it and send anyway, OR dispatch it to the
    client here. This flips the terminal status done -> needs_review by APPENDING a terminal line
    (never a rewrite), exactly as promote_to_done flips failed -> done, so blogState derives
    has_questions, clientCanSee turns true, and the topic enters the client 'Needs answers' tab
    and the rerun queue. It re-runs NO evaluator (first-score-is-final) and commits NO new
    version: the question form already sits in review_notes from the 95+ commit, current and
    unanswered, and portal_submit_answers has no status gate, so this is a visibility-only flip.

    The route (api_dispatch_question) owns every refusal: status is exactly done, the form is
    current and unanswered, no live run holds the topic, the blog is not approved or already out
    with the client. This function is the act plus the verify-landed safety promote_to_done
    pioneered. It does NOT ledger and does NOT send: the blog was already ledgered at its 95+
    ship, and it is going to the client to ANSWER, not to review.
    """
    # Scratch first, laid from the record, so the appended line lands after the record's own
    # high-water mark and commit_topic's (topic_id, line_no) keying reads it as genuinely new.
    sync.materialize_topic(client_slug, topic_slug)
    ...
    # score=None ON PURPOSE, exactly as promote_to_done: the flip touches the STATUS fold and
    # must not touch the SCORE fold, so the evaluator's 95+ stays intact beside a needs_review
    # verdict. iter is the iteration the draft is on, so _PENDING_SQL's asked_iter == max(iter)
    # match holds and the client's answer is accepted.
    runner._status_module().append_status(
        out_dir, topic_slug, stage="eval", event="end",
        iter=max(summary["iterations"], 1), score=None, status="needs_review",
        note=f"operator dispatched the evaluator's question to the client: "
             f"{email or 'an operator'} sent this blog (scored {summary['score']}) to the "
             f"client's Needs answers tab rather than shipping over the open question",
    )

    # allow_new_version=False: the draft the record already holds is the one the client answers
    # against, so stray scratch must never become a new version and move the form's anchor
    # (which would make the form its own stale and strand the client on it).
    sync.commit_topic(client_slug, topic_slug, allow_new_version=False)

    # VERIFY THE FLIP LANDED before anything rests on it, for promote_to_done's exact reason:
    # commit_topic keys status lines on (topic_id, line_no) ON CONFLICT DO NOTHING, so a disk
    # feed BEHIND the record (another engine ran this topic) appends at an ordinal the record
    # already owns and the flip is swallowed silently. Half a dispatch is worse than none: the
    # admin believes the client was asked while the client sees nothing. Re-read the fold and
    # refuse loudly if it did not land as needs_review.
    tid = db.topic_id(client_slug, topic_slug)
    landed = db.q(
        """select status from status_events
           where topic_id = %s and status <> 'running'
           order by line_no desc limit 1""",
        (tid,), fetch="val") if tid else None
    if landed != "needs_review":
        raise EditError(
            f"the dispatch did not reach the record: the fold still reads {landed!r}. This "
            f"machine's status feed for {topic_slug!r} is behind the record, which happens "
            f"when another engine ran the topic. Delete outputs/{client_slug}/{topic_slug}/"
            f"status.jsonl so it re-materializes from the record, then dispatch again."
        )

    # The NEEDS_REVIEW breadcrumb goes with the status it marks (see _enforce_terminal_status).
    # The blog is now held for the client's answer; a rerun landing back on done clears it.
    (out_dir / "NEEDS_REVIEW").write_text(
        f"{email or 'an operator'} dispatched the evaluator's question to the client to answer. "
