...
import type { RoadmapRow, StatusEvent } from "@/types";
// The fold moved to @/lib/run-fold so it can be tested without React; see its header.
// Re-exported because every existing caller imports these names from this module.
import { SEGMENTS, applyEvent, blank } from "@/lib/run-fold";
import type { Seed, SegState, Segment, TopicRun } from "@/lib/run-fold";

export { SEGMENTS, applyEvent, blank };
export type { Seed, SegState, Segment, TopicRun };
