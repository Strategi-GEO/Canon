  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <TabsContent value="new">
        ...
        {/* THE FAILED ROWS ARE IN THE PICK LIST ABOVE, not in a table of their own here.
            A previous pass gave them one, titled Ran without a verdict. The operator's answer to
            it was the right one: a failure is still a row on the sheet, so rerunning it belongs in
            the same list and the same selection as every other topic, rather than in a second
            table with a second way to choose things. select-state.tsx shownRows is where that
            lives now, and isSelectable has always allowed a failed row to be ticked. */}
