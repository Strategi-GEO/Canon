...
/**
 * CASE C: a 95+ blog that passed to internal review carrying an evaluator question. The admin
 * reads the question here and chooses: dispatch it to the client to ANSWER (this control, which
 * flips the blog to the client's Needs answers tab), or send the blog as it is for review (the
 * ordinary Send to client button in the delivery bar, which ships the question along unanswered).
 *
 * It is the read-only mirror of the Held panel: the same QuestionList, but no answer boxes,
 * because the admin is not the person who holds this fact, the client is. It renders only when
 * blog-stage's canGetItAnswered is true, which composes the internal_review bench with the gate
 * door, so it is absent for a Case D passed blog with no question and for an absent, stale or
 * answered form. The dispatch is api.dispatchQuestion; on success the page re-reads and derives
 * has_questions, and the blog moves to the client.
 */
export function PendingQuestionAtReview({
  brandSlug,
  topicSlug,
  entry,
  onDispatched,
  ...
  async function dispatch() {
    ...
    setError(null);
    try {
      await api.dispatchQuestion(brandSlug, topicSlug);
      toast.success("Sent to the client to answer");
      setOpen(false);
      onDispatched();
    } catch (cause) {
  ...
  return (
    <div className="border-b border-border bg-review-bg/40 px-4 py-3">
      <div className="flex items-start gap-2">
        <MessageCircleQuestion className="mt-0.5 size-4 shrink-0 text-review" aria-hidden />
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium text-review">
            This blog passed, but the evaluator raised a question only a person can settle.
          </p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            Send it to the client to answer and it moves to their Needs answers tab, then reruns
            with their answer. Or send the blog as it is from the bar above, and the question rides
            along unanswered.
          </p>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Send this question to the client?</DialogTitle>
                <DialogDescription>
                  The client sees the draft and answers in their portal. Answering reruns the blog
                  with their answer. The blog leaves your bench until they respond.
                </DialogDescription>
              </DialogHeader>
              {questions.length > 0 ? <QuestionList questions={questions} /> : null}
