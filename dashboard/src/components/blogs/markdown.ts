...
/**
 * eval.md carries "SCORE: NN" on its own line. The eval tab lifts it to the top, so an
 * operator reads the number without hunting for it.
 */
export function extractScore(evalText: string): number | null {
  const match = evalText.match(/^\s*SCORE:\s*(\d{1,3})\s*$/m);
  if (!match) {
    return null;
  }
