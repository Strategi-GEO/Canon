...
export function UploadBlog({
  ...
  // NO LONGER GATED ON HOSTED_READONLY. The commit is a database write, so migration 009's
  // admin_upload_blog performs it and app/api/clients/[slug]/blogs/[topic]/upload/route.ts
  // calls it. What the hosted build genuinely cannot do is RUN THE GATES: gates.py is a
  // subprocess and Vercel has no process to run it in. That difference is reported rather
  // than hidden, because the two are not the same fact: the function returns
  // `gates: {ran: false, reason}` and can never return `passed: true`, and GateReport below
  // renders "we did not check" instead of letting it read as "we checked and it was clean".
  // The scratch copy agents read is likewise engine-only, and it is regenerated by
  // sync.materialize_topic on the next local run, so its absence costs nothing here.
