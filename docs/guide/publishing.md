...
Every brand publishes to one of two places: the **Strategi CMS**, or the client's own website. Posting to the CMS files a draft that one of our editors reviews, so it releases nothing to anybody. Posting to the client's own website puts the article live on their domain, so it is the final release and it waits for the client's approval.
...
| Who can read it afterwards | A Strategi editor | Anyone on the internet |
| Is it a release? | No. An editor reviews it there and decides | Yes. There is no later step |
...
4. Fill in **Application password**. The card tells the client's side how to make one: "Users → Profile → Application Passwords → name it Canon → Add New. It is shown once." This is not their login password, and it can be revoked on their side at any time without changing their account.
5. Press **Connect**.
...
| The REST API is not reachable | A security plugin such as Wordfence, or the host, is blocking it | Ask whoever manages the site to allow requests to /wp-json/ |
...
Open the article from the **Blogs** tab. The **Post** control sits under the title, beside **Send to client**.
...
- The **Post** control goes away. A published article's only remaining control is **Unpublish**, and only on a website brand.
...
In the **Blogs** tab, tick the rows you want and use **Post to CMS** in the selection bar. It runs the same door one row at a time, and any blog the door will not accept is skipped rather than failing the batch. The bulk label reads "Post to CMS" whatever the brand's destination is, so check the destination on the settings card before using it on a website brand.
...
| No Post control, article tagged **Published** | It has already gone out | Nothing. A re published article is administered where it lives |
| Refused: no evaluator scored draft | The run never produced a scored article | Generate the topic again |
| Refused: the approval and the article have come apart | A new version was written after the client approved a different one | A person decides which is the real article. Send it again and have the client re approve |
...
- **Unpublishing keeps the address.** The recorded post and its URL are kept, so a re publish restores the article where it was instead of landing at a second address with a number on the end.
