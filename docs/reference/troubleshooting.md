...
When the dot is amber, the menu holds nothing but the problems, **Check again**, and **Quit**. That is deliberate: there is no dashboard to open yet. Once it is green the menu carries **Open dashboard**, **Restart**, **Check setup again**, **Open logs**, **Start at login** and **Quit**.

## Amber dot: what it can be

There are seven things that turn the dot amber, and the menu names whichever one applies in plain English. This table is what each one means.
...
| server/.env does not exist, or is missing required keys | The file holding the database credentials is absent or incomplete | Ask your admin for the current `server/.env`. Do not edit it by hand |
...
| --- | --- | --- |
| `session died in Ns having written no status line; not retried` | The session opened and died before it reached a single tool call. The usual cause is the Claude account's usage window being exhausted | Wait for the usage window to reset, then generate the topic again. Nothing was written, so nothing is lost |
...
The three names are `FIRECRAWL_API_KEY`, `DATAFORSEO_USERNAME` and `DATAFORSEO_PASSWORD`. They usually arrive with your Claude Code setup. If the engine says they are unset, ask your admin, put them in the same `server/.env` as everything else, and **Restart** from the menu.
...
| A blog shows **Generating** forever after you restarted Canon | The list of live runs lives in memory and does not survive a restart | The files on disk are intact. Refresh the page and read the blog's own status; generate it again if it never reached a verdict |
...
## Post to CMS is refused

| The message says | What it means | Fix |
| --- | --- | --- |
| No CMS write key configured for org *name* | That client org has no key on this machine | Add the line the message names to `server/.env`, then **Restart**. There is one key per org, never a shared one |
| This blog is `needs_review` | The evaluator asked a question and the blog is held until it is answered | Answer the questions. There is no dismiss |
| *topic* has not been approved by the client yet | The brand publishes to the client's own website, and posting there is the final release | Wait for the client to approve it in their portal. Posting to the Strategi CMS does not need approval, because a CMS draft is reviewed by an editor before it goes anywhere |
