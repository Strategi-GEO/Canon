...
The engine does name it, on every topic it refuses. A topic dispatched while dispatch is halted is refused before any session opens and lands terminal within seconds, carrying the engine's own note: that two sessions in a row died having written nothing, that the usual cause is the account's usage window, that nothing was spent on this topic, and how many minutes to wait. That sentence is in the topic's expanded row in the queue table and in its progress block on the **Create** tab, and the blog reads **Did not finish**. So what this looks like is blogs landing one after another in seconds carrying that note, not a queue that has stopped moving. Nothing was spent on them, so generate them again once a session runs.
